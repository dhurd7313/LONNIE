...
Rules:
1. If you need to use a tool: output the JSON block below. Nothing else.
2. If a tool already ran and failed: say exactly what the error was. Do not invent a result.
3. Never say "I found", "I searched", "I added" unless a tool actually ran and returned that data.

If you need a tool right now, output ONLY this (with real values):
\`\`\`json
{"tool":"tool_name","args":{"required_arg":"real value"}}
\`\`\`

Common arg mistakes to avoid:
- fetch_image needs: {"query": "descriptive search string"}
- write_file needs: {"filename": "name.ext", "content": "THE ACTUAL CONTENT"}
- run_js needs: {"code": "return expression;"}
- memory_store needs: {"key": "name", "value": "data"}
- agent_exec needs: {"command": "shell command"}

If you have nothing more to execute, just respond naturally about what happened.`;
